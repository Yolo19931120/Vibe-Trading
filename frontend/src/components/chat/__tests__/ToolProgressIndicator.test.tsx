...
describe("ToolProgressIndicator", () => {
  it("renders nothing when no tools are running", () => {
    ...
    const { container } = render(wrapWithI18n(<ToolProgressIndicator toolCalls={tcs} />));
    expect(container.innerHTML).toBe("");
  });

  it("renders nothing for empty array", () => {
    const { container } = render(wrapWithI18n(<ToolProgressIndicator toolCalls={[]} />));
    expect(container.innerHTML).toBe("");
  });

  it("renders single running tool", () => {
    const tcs = [makeTc({ elapsed_s: 5 })];
    render(wrapWithI18n(<ToolProgressIndicator toolCalls={tcs} />));
