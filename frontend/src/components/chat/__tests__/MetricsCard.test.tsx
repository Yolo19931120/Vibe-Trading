...
describe("MetricsCard", () => {
  ...
  it("renders nothing when metrics is empty", () => {
    const { container } = render(wrapWithI18n(<MetricsCard metrics={{}} />));
    expect(container.innerHTML).toBe("");
  });

  it("renders metric labels", () => {
    render(wrapWithI18n(<MetricsCard metrics={sampleMetrics} />));
  ...
  it("compact mode shows only first 6 metrics", () => {
    ...
    render(wrapWithI18n(<MetricsCard metrics={manyMetrics} compact />));
  ...
  it("ignores metrics not in DISPLAY_ORDER", () => {
    const { container } = render(wrapWithI18n(<MetricsCard metrics={{ unknown_metric: 999 }} />));
    expect(container.innerHTML).toBe("");
  });

  it("applies sentiment colors", () => {
    render(wrapWithI18n(<MetricsCard metrics={{ sharpe: 1.5 }} />));
  ...
  it("applies negative sentiment for bad values", () => {
    render(wrapWithI18n(<MetricsCard metrics={{ max_drawdown: -0.3 }} />));
