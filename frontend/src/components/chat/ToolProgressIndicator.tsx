  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-2 text-xs text-muted-foreground">
        {aggregateIcon}
        <span className="text-foreground">{t("streaming.toolsRunning", { count: running.length })}</span>
      </div>
      {/* Indented rows */}
      <div className="pl-4 space-y-1">
        ...
        {overflow > 0 && (
          <div className="flex items-center gap-2 text-[10px] text-muted-foreground/60">
            ...
            <span>{t("streaming.moreItems", { count: overflow })}</span>
          </div>
        )}
      </div>
    </div>
