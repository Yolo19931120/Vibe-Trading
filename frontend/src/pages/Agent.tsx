...
import { ApiError, api, isAuthRequiredError, type GoalSnapshot, type MandateProposal, type MandateCommitted, type LiveAction, type LiveHalted, type LiveStatus } from "@/lib/api";
    ...
    <div className="flex gap-3">
      ...
      <div className="flex-1 min-w-0">
        <div className={["inline-flex max-w-full flex-wrap items-center gap-1.5 rounded-lg border px-2.5 py-1 text-xs", tone].join(" ")}>
          ...
          <span className="shrink-0 font-medium uppercase tracking-wide text-[10px]">{t("connector.runtimeChip")}</span>
...
export function Agent() {
  ...
  useEffect(() => {
    onStatusChange((s) => {
      act().setSseStatus(s);
      if (s === "reconnecting" && prevSseStatusRef.current === "connected") toast.warning(t("common:toast.connectionLost"));
      else if (s === "connected" && prevSseStatusRef.current === "reconnecting") toast.success(t("common:toast.connectionRestored"));
  ...
  const loadGoalSnapshot = useCallback(async (sid?: string | null) => {
    ...
    try {
      ...
      if (error instanceof ApiError && error.status === 404) {
        ...
        toast.error(error instanceof Error ? error.message : t("goal.cancelFailed"));
      }
    }
  }, []);
  ...
  const setupSSE = useCallback((sid: string) => {
    ...
    connect(api.sseUrl(sid, { replay: "active" }), {
      ...
      "live.halted": (d) => {
        ...
        setLiveStatusRefresh((n) => n + 1);
        toast.warning(t("connector.haltedByKillSwitch"));
      },
      ...
      "live.resumed": (d) => {
        ...
        setLiveHalted(null);
        setLiveStatusRefresh((n) => n + 1);
        toast.success(t("connector.resumed"));
      },
  ...
  useEffect(() => {
    ...
    const timer = setInterval(() => {
      if (lastEventRef.current && Date.now() - lastEventRef.current > sseTimeoutMsRef.current && act().status === "streaming") {
        act().setStatus("idle");
        toast.warning(t("common:toast.executionTimedOut"));
      }
    }, 10_000);
    return () => clearInterval(timer);
  }, [status]);
  ...
  const runPrompt = async (prompt: string) => {
    ...
    if (goalComposerActive) {
      ...
      try {
        ...
        toast.success(t("goal.goalAttached"));
        ...
        act().setStatus("idle");
        toast.error(error instanceof Error ? error.message : t("goal.createFailed"));
      }
      return;
    }
    ...
    try {
      ...
      act().setStatus("error");
      const message = isAuthRequiredError(error) ? t("common:error.authRequired") : t("common:error.failedToSend");
      toast.error(message);
  ...
  const handleCancel = async () => {
    ...
    try {
      ...
      act().clearStreaming();
      useAgentStore.setState({ toolCalls: [] });
      toast.info(t("common:toast.cancelSent"));
    } catch {
      toast.error(t("common:toast.cancelFailed"));
    }
  };
  ...
  const handleHaltLive = useCallback(async () => {
    ...
    try {
      ...
      setLiveStatusRefresh((n) => n + 1);
      toast.success(t("connector.halted"));
    } catch (error) {
      toast.error(error instanceof Error ? error.message : t("connector.haltFailed"));
    } finally {
      setHalting(false);
    }
  }, [sessionId, halting]);
  ...
  const handleCancelGoal = useCallback(async () => {
    ...
    try {
      ...
      setGoalSnapshot(null);
      setGoalDetailsOpen(false);
      toast.success(t("goal.goalCancelled"));
    } catch (error) {
      toast.error(error instanceof Error ? error.message : t("goal.cancelFailed"));
    }
  ...
  const handleSaveGoalEdit = useCallback(async () => {
    ...
    try {
      ...
      setGoalEditActive(false);
      toast.success(t("goal.goalUpdated"));
    } catch (error) {
      toast.error(error instanceof Error ? error.message : t("goal.updateFailed"));
    }
  ...
  const handleContinueGoal = useCallback(async () => {
    ...
    try {
      ...
      act().setStatus("error");
      const message = isAuthRequiredError(error) ? t("common:error.authRequired") : t("goal.continueFailed");
      toast.error(message);
      act().addMessage({ id: "", type: "error", content: message, timestamp: Date.now() });
    }
  ...
  const handleExport = () => {
    ...
    const lines: string[] = [`# ${t("export.chatExport")}`, ``, `${t("export.exportTime")}: ${new Date().toLocaleString()}`, ``];
    for (const msg of messages) {
      const time = new Date(msg.timestamp).toLocaleString();
      if (msg.type === "user") {
        lines.push(`## ${t("export.user")} (${time})`, ``, msg.content, ``);
      } else if (msg.type === "answer") {
        lines.push(`## ${t("export.assistant")} (${time})`, ``, msg.content, ``);
      } else if (msg.type === "error") {
        lines.push(`## ${t("export.error")} (${time})`, ``, msg.content, ``);
      } else if (msg.type === "tool_call") {
        lines.push(`> ${t("export.toolCall")}: ${msg.tool || "unknown"}`, ``);
      } else if (msg.type === "swarm_status") {
        lines.push(`> ${t("export.swarmStatus")}: ${msg.swarmStatus?.preset || "swarm"} ${msg.swarmStatus?.status || ""}`, ``);
      } else if (msg.type === "run_complete") {
        lines.push(`> ${t("export.backtestComplete")}: ${msg.runId || ""}`, ``);
      }
    }
  ...
  const handleFileSelect = useCallback(async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (blockedExts.some((ext) => lowered.endsWith(ext))) {
      toast.error(t("common:toast.fileBlocked"));
      return;
    }
    if (file.size > 50 * 1024 * 1024) {
      toast.error(t("common:toast.fileTooLarge"));
      return;
    }
    setUploading(true);
    ...
    try {
      ...
      toast.success(t("toast.uploadSuccess", { filename: result.filename }));
    } catch (err) {
      toast.error(t("toast.uploadFailed", { message: err instanceof Error ? err.message : "Unknown error" }));
    } finally {
      setUploading(false);
    }
  }, []);
        ...
        <div className="max-w-3xl mx-auto space-y-4">
          ...
          {status === "streaming" && !streamingText && toolCalls.length === 0 && !messages.some((m) => m.type === "swarm_status" && m.swarmStatus?.status === "running") && (
            <div className="flex gap-3">
              ...
              <div className="flex-1 min-w-0 flex items-center gap-2 text-xs text-muted-foreground pt-1">
                ...
                <span>{t("streaming.agentWorking")}</span>
              </div>
            </div>
          )}
          ...
          {status === "streaming" && (
            <div className="flex items-center gap-2 px-1 pt-1">
              ...
              <span className="text-[10px] text-muted-foreground shrink-0 tabular-nums">{t("connector.runningChip")}</span>
            </div>
          )}
              ...
              >
                ...
                {goalProgress.total > 0 && (
                  <span className="shrink-0 font-mono text-[11px] text-emerald-600 dark:text-emerald-400">
                    {t("goal.met", { met: goalProgress.met, total: goalProgress.total })}
                  </span>
                )}
                ...
                <div className="absolute bottom-full left-0 mb-2 w-52 rounded-xl border bg-background/95 backdrop-blur-sm shadow-lg py-1 z-50">
                  ...
                  <button
                    ...
                    onClick={() => {
                      setShowUploadMenu(false);
                      setGoalComposerActive(false);
                      setSwarmPreset({ name: "auto", title: t("input.swarmMode") });
                      inputRef.current?.focus();
                    }}
                    className="w-full px-3 py-2 text-left text-sm hover:bg-muted transition-colors flex items-center gap-2"
                  >
